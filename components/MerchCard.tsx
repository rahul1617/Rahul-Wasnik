...
const MerchCard: React.FC<MerchCardProps> = ({ item }) => {
  return (
    <div className="group bg-[#121212] rounded-[2rem] overflow-hidden flex flex-col h-full border border-white/5 hover:border-[#38BDF8]/30 transition-all hover:shadow-[0_0_30px_rgba(0,0,0,0.2)]">
      ...
      <div className="relative h-72 bg-[#080808] p-8 flex items-center justify-center overflow-hidden">
        ...
        <img 
          ...
          decoding="async"
        />
        <div className="absolute top-4 left-4 bg-[#38BDF8] text-black text-[10px] font-bold px-3 py-1 rounded-full opacity-0 group-hover:opacity-100 transition-opacity transform -translate-y-2 group-hover:translate-y-0 duration-300">
            NEW ARRIVAL
        </div>
      </div>
      ...
      <div className="p-6 flex flex-col flex-grow relative border-t border-white/5">
        ...
        <a href={item.buyLink} target="_blank" rel="noopener noreferrer" className="mt-auto">
            <Button variant="outline" className="w-full gap-2 text-xs rounded-full border-white/10 text-slate-300 hover:text-[#38BDF8] hover:border-[#38BDF8] hover:bg-[#38BDF8]/5 group-hover:shadow-lg">
